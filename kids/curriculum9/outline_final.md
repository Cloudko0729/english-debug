...
**Lv.7 / Lv.8 / Lv.9 已做完上線（2026-07-21，K9 全 9 級完工）**：`k9/tools/render_lv79.js`，8+8+8週，408個音檔。使用者看到一篇講「情境→文法→同一文法反覆出現在故事裡」的貼文，問可不可以參考；確認後決定套用在國中三級——**用一個貫穿三年的主角 Zoe（+同學 Leo）當故事線**，24 週的對話都是同一對角色的連續情節（七年級新生活→八年級專題→九年級畢業發表），不是像 Lv.1-6 那樣每週換一批隨機角色。
- **文法內容全新設計**（grammar_core 完全沒有這些，早先已拍板採 Codex 完整版方案）：Lv.7＝比較級/最高級、可數不可數量詞、to-infinitive、when/while、if 入門；Lv.8＝should/must/might、第一條件句正式版、關係子句(who/that/which)、現在完成式經驗用法；Lv.9＝現在完成式vs過去式、被動語態(流程)、引述觀點(according to/said that)、篇章銜接詞(however/therefore/in addition)。每週還是同一套「📐文法小提示」卡格式（概念+3-4例句），跟 Lv.4-6 一致。
- **字彙**：Lv.7 直接用 wL7 全部 575 字；Lv.8/Lv.9 平分 wL8（346字 extra list）各 173 字——**這只是先對半切的 MVP**，還沒補教育部7000字表擴充，是延續原本就記錄的開放問題，之後要再議。wL7 有171字、wL8有8字 wordbank.js沒收錄，補了 EXTRA_ZH 翻譯（無缺漏，程式驗證過）。
- 沒有做 Lv.7-9 的單字練習題（vocab_quiz.html）——那是 Lv.1-6 另外被要求加的功能，這次使用者只要求「套用故事線設計」，沒有要求同步加練習題，範圍上刻意沒有擴大；如果之後要加，wL7(575)+wL8(346)=921 字都要新寫例句，是接近 Lv.1-6 那次同等規模的工作量。
- 已用 Playwright 跑過全部 27 個新頁面（3個index+24週），console 全乾淨、408 個音檔全部存在、字彙池 921 字（575+173+173）零翻譯缺口。
- 根目錄 `k9/index.html` 9 個等級全部標「已上線」，K9 Lv.1-9 大綱從規劃到完工至此全部完成。

**Lv.7 單字練習題（2026-07-21）**：使用者接著要求先做 Lv.7 的單字練習題（Lv.8/9 之後再說）。`pool_sentences.js` 新增 575 句 wL7 原創例句（程式驗證過，一開始漏了 `bug` 一個字，補上後零缺漏），`render_pool_quiz.js` 擴充涵蓋 Lv.7，`render_lv79.js` 的 Lv.7 index 頁補回「🎯 單字練習題」連結（Lv.8/9 沒有，等之後真的做才加）。1150 個新音檔（575字×單字+例句各一個檔）背景生成。Playwright 驗證：quiz 頁兩顆發音鍵各自打對音檔、index 頁零翻譯缺口、作答流程正常。

**Lv.4-6 字彙池落差已修復（2026-07-21）**：檢查 Lv.7 字彙池範圍時發現 Lv.1-6（尤其 Lv.4/5/6）的字彙池只用了 `WORD_LEVELS`（PDF 1200字表）子集，沒有合併 `EXTRA_LEVELS`（Codex 額外分級字）——導致 Lv.4/5/6 實際字彙池只有 107/193/243 字，比大綱一開始講的「累計137/343/552」少了 30/150/309 字。使用者確認要修：
- `render_lv46.js`／`render_pool_quiz.js` 的字彙池範圍改用 `wordLevel()`（WORD_LEVELS+EXTRA_LEVELS 合併查詢，跟 Lv.7 同一套邏輯），Lv.4-6 現在正確顯示 137/343/552 字，跟大綱原本的累計數字一致。Lv.1-3 維持原本 WORD_LEVELS-only 範圍不變（額外字很少，且不影響已上線內容）。
- `pool_sentences.js` 新增 489 句例句（Lv.4=30、Lv.5=150、Lv.6=309），程式驗證零缺漏；`render_lv46.js` 的 `EXTRA_ZH` 補了 45 個新收字的中文翻譯，零翻譯缺口。
- 978 個新音檔（489字×單字+例句各一）背景生成。Playwright 驗證 Lv.4/5/6 的 index 頁字數正確、零翻譯缺口，vocab_quiz 頁面正常作答無誤。

**字彙池 UI 改版（同日）**：使用者要求字彙池預設收合、或移到頁尾。做法：把字彙池從一般 `<div class="card">` 改成 `<details class="card poolcard">`（預設收合，只顯示一行摘要），並且整段移到週次列表**之後**（頁尾）——兩個要求都做了。套用到全部 9 級（render_lv1/23/46/79.js 都改）。Playwright 驗證全 9 級：`details.poolcard` 沒有 `open` 屬性（預設收合）且在週次列表 DOM 順序之後。

...
**發現一個既有落差（未修）**：檢查 Lv.7 字彙池範圍時發現 Lv.1-6（尤其 Lv.4/5/6）的字彙池只用了 `WORD_LEVELS`（PDF 1200字表）子集，沒有合併 `EXTRA_LEVELS`（Codex 額外分級字）——導致 Lv.4/5/6 實際字彙池只有 107/193/243 字，比大綱一開始講的「累計137/343/552」少了 30/150/309 字。Lv.7 這次改用 `wordLevel()` 合併查詢做對了（575字含PDF+extra）。**這個落差還沒修**，要修的話 Lv.4-6 要多補489個字的例句才能同步放進單字練習題，是不小的工作量，留給使用者決定要不要處理。

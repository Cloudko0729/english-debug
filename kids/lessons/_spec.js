// 用法: node _spec.js <date> — 從教學頁抽 VOCAB/DIALOGUE/PASSAGE + play('gN',...) 文法句，產音檔規格
const fs = require("fs");
...
let m, re = /play\('g(\d+)','([^']*)'\)/g;
while ((m = re.exec(h)) !== null) items["g" + m[1]] = m[2];
...
console.log(date, "→", Object.keys(items).length, "段（含文法 g）");

...
function ans(i, id, btn) {
  ...
  if (done === QUIZ.length) document.getElementById("quizScore").textContent = "🎉 答對 " + ok + " / " + QUIZ.length;
}
...
${bands.map(b => {
  ...
  <div class="nlist">
    ${byBand[b].map(n => `<a href="${esc(n.id)}.html"><span>${esc(n.titleZh)}<span class="sub"> ${esc(n.titleEn)}</span></span><span>›</span></a>`).join("")}
  </div>
</div>`;
  }).join("")}
</body>

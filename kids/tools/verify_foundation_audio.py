"""Decode-check every MP3 path referenced by the foundation vocabulary DB."""
...
def main() -> None:
    paths: set[str] = set()
    for filename in DB_DIR.glob("*.json"):
    ...
    if errors:
        print(f"Foundation audio verification failed ({len(errors)}):", file=sys.stderr)
        for error in errors:
